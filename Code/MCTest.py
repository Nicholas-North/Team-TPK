...
class MonteCarloSimulation:
    def __init__(self, num_simulations=100):
        ...
        self.players = create_classes()  # Assuming this is a function that returns all available players
        
        if not isinstance(self.players, list) or len(self.players) == 0:
            raise ValueError("Error: create_classes() did not return a valid list of players.")
        
        # Call select_players from MonteCarloSimulation
        self.selected_players = self.select_players(self.players)
        
    def select_players(self, all_players):
        """Allow the user to select three players for the team and one solo player."""
        ...
        while True:
            try:
                team_indices = input("Select three players for the team (enter three numbers separated by a comma): ").split(',')
                team_indices = [int(i.strip()) - 1 for i in team_indices]
                ...
                if all(0 <= i < len(all_players) for i in team_indices + [solo_index]):
                    team_players = [all_players[i] for i in team_indices]
                    solo_player = all_players[solo_index]
                    return team_players + [solo_player]
    ...
    def display_results(self):
        print("\nFinal Results After 100 Simulations:")
        for team, wins in self.results.items():
            print(f"{team}: {wins} wins")

# Modify CombatSimulation to return the winner
class CombatSimulation:
    def __init__(self, players):
        self.templates = initialize_templates()
        self.players = players  # Use passed players list directly
        
        # Select the first 4 players from the list, or customize this part
        self.selected_players = self.players[:4]
        
        if len(self.selected_players) != 4:
            raise ValueError("Error: Not enough players selected.")
        
        self.team_players = self.selected_players[:3]
        self.solo_player = self.selected_players[3]
        
        self.original_stats = {player.name: {
            "hit_points": player.hit_points,
            "num_heals": player.num_heals
        } for player in self.players}

        
    def select_players(self, all_players):
        """Allow the user to select three players from the available options, with one solo player."""
        print("Available Players:")
        for i, player in enumerate(all_players):
            print(f"{i + 1}: {player.name} ({player.player_class})")
        ...
        while True:
            try:
                ...
                if all(0 <= i < len(all_players) for i in team_indices + [solo_index]):
                    ...
                    return self.team_players + [self.solo_player]  # Return only team players list (self.solo_player will be used directly)
    ...
    def reset_players(self):
        if not hasattr(self, 'original_stats') or not self.original_stats:
            print("Error: original_stats is not initialized or empty.")
            return
    
        for player in self.players:
            if player.name in self.original_stats:
                print(f"Resetting {player.name}:")
                print(f"Before -> HP: {player.hit_points}, Heals: {player.num_heals}")
            
                player.hit_points = self.original_stats[player.name]["hit_points"]
                player.num_heals = self.original_stats[player.name]["num_heals"]
            
            print(f"After -> HP: {player.hit_points}, Heals: {player.num_heals}")

    def run_round(self):
        self.reset_players()  # Reset player stats before starting a new combat
        while self.solo_player.hit_points > 0 and any(p.hit_points > 0 for p in self.team_players):
            ...
            if self.solo_player.hit_points > 0:
                target = random.choice([p for p in self.team_players if p.hit_points > 0])
                self.perform_actions(self.solo_player, target)
                if all(p.hit_points <= 0 for p in self.team_players):
                    return "Solo Player Wins"
